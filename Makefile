...
# Disable LTO when using GPU (nvcc doesn't handle it well)
ifeq ($(USE_GPU),1)
LTO_FLAGS :=
else
LTO_FLAGS := -flto
...
# GPU settings
NVCC := nvcc
CUDA_CFLAGS := -O3 -Iinclude -DDEBUG=$(DEBUG) -DUSE_GPU=1 -Xcompiler -Wall -Xcompiler -Wextra
...
ifeq ($(USE_GPU),1)
ifeq ($(PGO),gen)
$(error PGO is not supported with GPU builds)
endif
ifeq ($(PGO),use)
$(error PGO is not supported with GPU builds)
endif
